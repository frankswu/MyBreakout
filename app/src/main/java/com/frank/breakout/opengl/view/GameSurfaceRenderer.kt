...
class GameSurfaceRenderer : GLSurfaceView.Renderer {
    ...
    private var mViewportXoff = 0
    private var mViewportYoff: Int = 0

    private var mSurfaceView: GameSurfaceView? = null
    private var mGameState: GameState? = null
    private var mTextConfig: TextResources.Configuration? = null

    companion object {
        const val EXTRA_CHECK = true // enable additional assertions

        // Orthographic projection matrix.  Must be updated when the available screen area
        // changes (e.g. when the device is rotated).
        public val mProjectionMatrix = FloatArray(16)
    ...
    override fun onSurfaceCreated(unused: GL10?, config: EGLConfig?) {
        if (EXTRA_CHECK) OpenGLUtil.checkGlError("onSurfaceCreated start")
        ...
        with(mGameState!!) {
            setTextResources(TextResources(mTextConfig!!))
            allocBorders()
            allocBricks()
            allocPaddle()
            allocBall()
        ...
        if (EXTRA_CHECK) OpenGLUtil.checkGlError("onSurfaceCreated end")
    }
    ...
    override fun onSurfaceChanged(unused: GL10?, width: Int, height: Int) {
         ...
         */
        if (EXTRA_CHECK) OpenGLUtil.checkGlError("onSurfaceChanged start")
        val arenaRatio: Float = GameState.ARENA_HEIGHT / GameState.ARENA_WIDTH
        val x: Int
        val y: Int
        val viewWidth: Int
        val viewHeight: Int
        if (height > (width * arenaRatio).toInt()) {
            // limited by narrow width; restrict height
            viewWidth = width
            viewHeight = (width * arenaRatio).toInt()
        } else {
            // limited by short height; restrict width
            viewHeight = height
            viewWidth = (height / arenaRatio).toInt()
        }
        x = (width - viewWidth) / 2
        ...
        Matrix.orthoM(
            mProjectionMatrix, 0, 0F, GameState.ARENA_WIDTH,
            0F, GameState.ARENA_HEIGHT, -1f, 1f
        )

        // Nudge game state after the surface change.
        mGameState?.surfaceChanged()
        if (EXTRA_CHECK) OpenGLUtil.checkGlError("onSurfaceChanged end")
    }
    ...
    override fun onDrawFrame(unused: GL10?) {
        ...
        TexturedAlignedRect.drawContent {
            gameState.drawScore().drawBall().drawMessages()
        }
        ...
        if (EXTRA_CHECK) OpenGLUtil.checkGlError("onDrawFrame end")
     ...
     */
    fun onViewPause(syncObj: ConditionVariable) {
        /*
    ...
    fun touchEvent(x: Float, y: Float) {
         ...
         * touches here.
         */
        val arenaX: Float = (x - mViewportXoff) * (GameState.ARENA_WIDTH / mViewportWidth)
        //val arenaY: Float = (y - mViewportYoff) * (GameState.ARENA_HEIGHT / mViewportHeight)
        Log.v(TAG, "touch at x=$x y=$y+  --> arenaX=$arenaX");
